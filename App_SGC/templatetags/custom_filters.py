...
def format_cnpj(value):
    ...
    # Aplica a máscara de CNPJ de acordo com o número de dígitos
    if len(cnpj) == 14:
        return '{}.{}.{}/{}-{}'.format(cnpj[:2], cnpj[2:5], cnpj[5:8], cnpj[8:12], cnpj[12:])
    elif len(cnpj) == 13:
        return '{}.{}.{}/{}-{}'.format(cnpj[:1], cnpj[1:4], cnpj[4:7], cnpj[7:11], cnpj[11:])

    # Retorna o valor original se o CNPJ não tiver 13 ou 14 dígitos
    return value
...
# Formata números xxx.xxx.xxx
@register.filter
def format_number(value):
    """
    Formata o número no formato xxx.xxx.xxx.
    """
    try:
